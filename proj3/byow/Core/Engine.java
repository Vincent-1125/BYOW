...
public class Engine {
    ...
    private void moveWithKeyboard(TERenderer ter) {
        while (true) {
            if (StdDraw.hasNextKeyTyped()) {
                char c = Character.toUpperCase(StdDraw.nextKeyTyped());
                if (c == ':') {
    ...
    private boolean validMove(char c) {
        if (c == 'W') {
            return tiles[agent.x][agent.y + 1] == Tileset.FLOOR;
        } else if (c == 'S') {
            return tiles[agent.x][agent.y - 1] == Tileset.FLOOR;
        } else if (c == 'A') {
            return tiles[agent.x - 1][agent.y] == Tileset.FLOOR;
        } else if (c == 'D') {
            return tiles[agent.x + 1][agent.y] == Tileset.FLOOR;
        }
        return false;
    ...
    private void generateRandomRooms(Long seed, int width, int height) {
        ...
        generateHallways(rooms, roomNum);
        buildWalls();
    }
